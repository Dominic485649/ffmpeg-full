...
- 包含 Apple AudioToolbox 的 `aac_at` 编码器，可用 `-c:a aac_at`；所需 Apple Application Support 运行时 DLL 随发布包提供。
...
> 构建包含 Apple 运行时和其他 nonfree 组件，FFmpeg 会将二进制标记为 `nonfree and unredistributable`。请自行确认使用与再分发合规性。
